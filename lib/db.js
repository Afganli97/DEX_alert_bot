...
async function connectToMongo(uri) {
  ...
  try {
    ...
    await db.collection('alerts').createIndex({ source: 1, status: 1 }, { unique: false, background: true });
    await db.collection('alerts').createIndex({ ownerId: 1 }, { unique: false, background: true });
