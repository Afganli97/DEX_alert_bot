  ...
  const doc = await usersCollection.findOneAndUpdate(
    { _id: chatId },
    {
      $setOnInsert: {
        ...
        status: 'active',
        subscription: 'basic',
        // lastActivityAt не задаём здесь – будет установлено через $currentDate
      },
      $currentDate: { lastActivityAt: true }, // безопасное обновление даты
      $set: { username: username || null }, // обновляем username отдельно
    },
