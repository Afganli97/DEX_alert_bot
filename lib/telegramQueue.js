...
class TelegramQueue {
  ...
  async sendTelegramTo(chatId, text) {
    try {
      ...
      // TODO: 429 здесь блокирует всю очередь на время retry_after,
      // а не только сообщение проблемному получателю. При росте
      // количество юзеров рассмотреть вынос проблемного сообщения
      // в конец очереди вместо блокирующего ожидания.
      if (data.error_code === 429 && data.parameters?.retry_after) {
        const retryAfter = Math.min(data.parameters.retry_after, 60); // Ограничим до 60 секунд для защиты от DOS
        // Move this message to the end of the queue instead of blocking
        this.queue.push({ chatId, text, resolve, reject });
        return false;
      }
...
module.exports = { TelegramQueue, sleep };