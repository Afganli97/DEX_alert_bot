...
class TelegramQueue {
  ...
  async process() {
    ...
    while (this.queue.length) {
      ...
      try {
        const result = await this.sendTelegramTo(chatId, text);
        if (result && result.retry) {
          // 429: re-queue this message with the same resolve/reject
          const retryAfter = Math.min(result.retryAfter, 60); // Cap at 60s to protect against DOS
          this.queue.push({ chatId, text, resolve, reject });
          // Wait only for the rate-limit window
          await sleep(retryAfter * 1000);
          // Don't resolve here - the re-queued attempt will resolve
        } else {
          // true = sent, false = failed permanently
          resolve(result);
        }
      } catch (err) {
   ...
   * @param {string|number} chatId
   * @param {string} text
   * @returns {Promise<true|false|{retry: true, retryAfter: number}>}
   *   true = sent, false = failed permanently, {retry: true, retryAfter} = retry pending (429)
   */
  async sendTelegramTo(chatId, text) {
    try {
      ...
      // 429: signal retry with retry_after
      if (data.error_code === 429 && data.parameters?.retry_after) {
        return { retry: true, retryAfter: data.parameters.retry_after };
      }
